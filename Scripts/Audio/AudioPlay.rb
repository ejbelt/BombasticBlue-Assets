...
def pbResolveAudioFile(str, volume = nil, pitch = nil)
  ...
  if str.is_a?(String)
    ...
    if (AudioCache.GetCacheValue(str) != nil)
      cache_member = AudioCache.GetCacheValue(str)
      puts cache_member.name
      puts cache_member.audio_file
      return cache_member.audio_file
    end
  ...
  puts "STR not string or audio file"

  if (str.is_a?(AudioCacheMember))
    puts str.name
    str.audio_file.volume = volume || 100
    str.audio_file.pitch = pitch || 100
...
#Tries to load the sound, playing at 0 volume and pitch before stopping it almost immediately to trick the engine into thinking it was played before
#I don't know if this will work as intended but hopefully?

#Tested, this actually works!
def bbPreloadSE(param)
  return if !param
  param = pbResolveAudioFile(param, 0, 0)
  if param.name && param.name != ""
    if $game_system
      $game_system.se_play(param)
      $game_system.se_stop()
